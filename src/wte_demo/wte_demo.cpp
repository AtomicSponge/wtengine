...
void wte_demo::load_menus(void) {
...

    {
        ...
        temp_menu.add_item(wte_menu_setting("Lives:", "max_lives", lives_vec, false));
...

    {
        ...
        temp_menu.add_item(wte_menu_setting("Scale factor:", "scale_factor", scale_vec, true));
...
void wte_demo::new_game(void) {
    ...
    wte_new_component(e_id, cmp::background, mgr::render_manager::get_arena_width(),
        ...
        [](entity bkg_id, mgr::entity_manager& world, int64_t engine_time) {
            ...
            al_set_target_bitmap(wte_get_component(cmp::background, bkg_id)->background_bitmap);
            ...
            for(std::size_t i = 0; i < MAX_STARS; i++) {
                ...
                if(wte_get_component(stars, bkg_id)->color[i] == 2)
                    al_draw_pixel(wte_get_component(stars, bkg_id)->x[i],
                                  wte_get_component(stars, bkg_id)->y[i], WTE_COLOR_YELLOW);
                if(wte_get_component(stars, bkg_id)->color[i] == 3)
                    al_draw_pixel(wte_get_component(stars, bkg_id)->x[i],
                                  wte_get_component(stars, bkg_id)->y[i], WTE_COLOR_RED);
            }
        }
    ...
    wte_new_component(e_id, cmp::dispatcher,
        [](entity bkg_id, mgr::entity_manager& world, message msg) {
            ...
            if(msg.get_cmd() == "default") wte_set_component(stars, bkg_id)->speed_mult = 1;
            if(msg.get_cmd() == "up") wte_set_component(stars, bkg_id)->speed_mult *= 2;
            if(msg.get_cmd() == "down") wte_set_component(stars, bkg_id)->speed_mult /= 2;
    ...
    wte_new_component(e_id, cmp::overlay, 200, 20, 0, mgr::render_manager::get_arena_height() - 20, 0,
        [](entity ovr_id, mgr::entity_manager& world, int64_t engine_time) {
            ...
            al_set_target_bitmap(wte_get_component(cmp::overlay, ovr_id)->overlay_bitmap);
            al_clear_to_color(WTE_COLOR_TRANSPARENT);
            wte_set_component(cmp::overlay, ovr_id)->set_text("Score:  ", WTE_COLOR_WHITE, 110, 0, ALLEGRO_ALIGN_RIGHT);
            wte_set_component(cmp::overlay, ovr_id)->set_text(game_cfg::get("score"), WTE_COLOR_WHITE, 110, 0, ALLEGRO_ALIGN_LEFT);
            wte_set_component(cmp::overlay, ovr_id)->set_text("High Score:  ", WTE_COLOR_WHITE, 110, 10, ALLEGRO_ALIGN_RIGHT);
            wte_set_component(cmp::overlay, ovr_id)->set_text(game_cfg::get("hiscore"), WTE_COLOR_WHITE, 110, 10, ALLEGRO_ALIGN_LEFT);
        }
    );
    wte_set_component(cmp::overlay, e_id)->set_font(al_create_builtin_font());
    ...
    wte_new_component(e_id, cmp::overlay, 200, 20, mgr::render_manager::get_arena_width() - 200,
        ...
        [](entity ovr_id, mgr::entity_manager& world, int64_t engine_time) {
            ...
            wte_set_component(cmp::overlay, ovr_id)->set_text("Shield", WTE_COLOR_WHITE, 200, 0, ALLEGRO_ALIGN_RIGHT);
            wte_set_component(cmp::overlay, ovr_id)->set_text("Lives:  " + game_cfg::get("lives"), WTE_COLOR_WHITE, 200, 10, ALLEGRO_ALIGN_RIGHT);
        }
    );
    wte_set_component(cmp::overlay, e_id)->set_font(al_create_builtin_font());
    ...
    wte_set_component(cmp::sprite, e_id)->load_sprite("ship.bmp", ".bmp");
    wte_set_component(cmp::sprite, e_id)->add_cycle("main", 0, 3);
    wte_set_component(cmp::sprite, e_id)->set_cycle("main");
    ...
    wte_set_component(cmp::sprite, e_id)->load_sprite("shield.bmp", ".bmp");
    wte_set_component(cmp::sprite, e_id)->add_cycle("main", 0, 5);
    wte_set_component(cmp::sprite, e_id)->set_cycle("main");
    ...
    wte_set_component(cmp::sprite, e_id)->load_sprite("cannon.bmp", ".bmp");
    wte_set_component(cmp::sprite, e_id)->add_cycle("main", 0, 3);
    wte_set_component(cmp::sprite, e_id)->set_cycle("main");
        ...
        [](entity e_id, mgr::entity_manager& world, msg_arg_list args) {
            ...
            wte_new_component(e_id, cmp::ai,
                [](entity ast_id, mgr::entity_manager& world, mgr::message_manager& messages, int64_t engine_time) {
                    ...
                    wte_set_component(cmp::location, ast_id)->pos_x +=
                        wte_get_component(cmp::velocity, ast_id)->speed *
                        cos(wte_get_component(cmp::direction, ast_id)->angle * (M_PI / 180));

                    wte_set_component(cmp::location, ast_id)->pos_y +=
                        wte_get_component(cmp::velocity, ast_id)->speed *
                        sin(wte_get_component(cmp::direction, ast_id)->angle * (M_PI / 180));

                    //  Perform OOB check.
                    if(wte_get_component(cmp::location, ast_id)->pos_y > mgr::render_manager::get_arena_height() + 100) {
                        messages.add_message(message("spawner", "delete",
                                             wte_get_component(cmp::name, ast_id)->name_str));
                    }

                    //  Health check.  If asteroid's HP is <= 0, reward player with points and delete the entity.
                    if(wte_get_component(cmp::health, ast_id)->hp <= 0) {
                        messages.add_message(message("spawner", "delete", wte_get_component(cmp::name, ast_id)->name_str));
            ...
            wte_new_component(e_id, cmp::dispatcher,
                [](entity ast_id, mgr::entity_manager& world, message msg) {
                    ...
                    if(msg.get_cmd() == "colision") {
                        ...
                        if(msg.get_from() == "main_cannon") wte_set_component(cmp::health, ast_id)->hp -= 1;
                        //  Shield hit the enemy.
                        if(msg.get_from() == "shield") wte_set_component(cmp::health, ast_id)->hp -= 10;
                    } //  end colision messages
                }
            );
            wte_new_component(e_id, cmp::sprite, 16, 16, 0.0f, 0.0f, 10, 0);
            wte_set_component(cmp::sprite, e_id)->load_sprite("asteroid.bmp", ".bmp");
            wte_set_component(cmp::sprite, e_id)->add_cycle("main", 0, 5);
            wte_set_component(cmp::sprite, e_id)->set_cycle("main");
            wte_set_component(cmp::sprite, e_id)->scale_factor_x = (float)size;
            wte_set_component(cmp::sprite, e_id)->scale_factor_y = (float)size;
        }
    );
