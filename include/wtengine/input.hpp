...
class input {
    ...
    private:
        ...
        constexpr inline static void run_handles(const ALLEGRO_EVENT& event) {
            //  Keyboard events
            if constexpr (handler_register<S, WTE_EVENT_KEY_DOWN>::is_set) {
            ...
            if constexpr (handler_register<S, WTE_EVENT_KEY_UP>::is_set) {
                static_assert(build_options.keyboard_enabled, "Keyboard not enabled in build options.");
            ...
            //  Mouse events
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_AXIS>::is_set) {
            ...
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_BUTTON_DOWN>::is_set) {
                static_assert(build_options.mouse_enabled, "Mouse not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_BUTTON_UP>::is_set) {
                static_assert(build_options.mouse_enabled, "Mouse not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_WARPED>::is_set) {
                static_assert(build_options.mouse_enabled, "Mouse not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_ENTER_DISPLAY>::is_set) {
                static_assert(build_options.mouse_enabled, "Mouse not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_MOUSE_LEAVE_DISPLAY>::is_set) {
                static_assert(build_options.mouse_enabled, "Mouse not enabled in build options.");
            ...
            //  Joystick events
            if constexpr (handler_register<S, WTE_EVENT_JOYSTICK_AXIS>::is_set) {
            ...
            if constexpr (handler_register<S, WTE_EVENT_JOYSTICK_BUTTON_DOWN>::is_set) {
                static_assert(build_options.joystick_enabled, "Joystick not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_JOYSTICK_BUTTON_UP>::is_set) {
                static_assert(build_options.joystick_enabled, "Joystick not enabled in build options.");
            ...
            //  Touch events
            if constexpr (handler_register<S, WTE_EVENT_TOUCH_BEGIN>::is_set) {
            ...
            if constexpr (handler_register<S, WTE_EVENT_TOUCH_END>::is_set) {
                static_assert(build_options.touch_enabled, "Touch not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_TOUCH_MOVE>::is_set) {
                static_assert(build_options.touch_enabled, "Touch not enabled in build options.");
            ...
            if constexpr (handler_register<S, WTE_EVENT_TOUCH_CANCEL>::is_set) {
                static_assert(build_options.touch_enabled, "Touch not enabled in build options.");
